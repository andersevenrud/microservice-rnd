    ...
    if prod:
        ...
        docker_build(
            ...
            live_update=[
                ...
                sync("packages/%s" % name, "/usr/src/%s" % name),
            ],
        )
...
k8s_resource(
    workload="topic-migrations", labels="jobs", trigger_mode=TRIGGER_MODE_MANUAL
)
k8s_resource(
    workload="keycloak-migrations", labels="jobs", trigger_mode=TRIGGER_MODE_MANUAL
)
